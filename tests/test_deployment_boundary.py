...
def test_inert_service_audit_records_partial_evidence_without_activation() -> None:
    ...
    assert "Exact 0.2.1 inert service update complete" in audit
    assert "post-deployment evidence\n  publication approved; activation blocked" in audit
    assert "approved all\n  five deployment-evidence publication items" in audit
    assert "provider-mutation approval" in audit
    assert "dashboard still showed maintenance mode enabled" in audit
    assert "VIASIGN_CORS_ORIGINS` is empty" in audit
    assert "HTTP 200" in audit
    assert '{"status":"ok"}' in audit
    assert "prior `e8ca6a6` parser `0.2.0` deployment" in audit
    assert "rollback rehearsal remains unapproved" in audit
