...
those values. Parser `0.2.1` source and its exact local deployment-candidate
evidence are published. The reviewed `0.2.1` commit is now live only inside the
inert service, and its post-deployment evidence has separate publication
approval. DNS, production CORS, website connection, maintenance-mode removal,
visitor submissions, and public activation remain blocked.
